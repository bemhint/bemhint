...
### Вложенные папки уровней переопределения

Если уровни переопределения расположены в папках вида `blocks/{levelName}`, то:

- В массиве `levels` файла конфигурации указываем только `{levelName}`, напр.:
```js
levels: [
    'root',
    'shared',
...
```
- При запуске команды в параметре `TARGETS` командной строки указываем расположение папки-контейнера (`blocks`):
```shell
$ bemhtml blocks
```
