...
В массиве `levels` файла конфигурации указыватся только `{levelName}` (basename), напр.:
```js
levels: [
    'root',
    'shared',
...
```
В параметре командной строки `TARGETS` указывается расположение папки-контейнера (`blocks`):
```shell
$ bemhtml blocks
```
