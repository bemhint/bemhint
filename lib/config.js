...
var Config = inherit({
    ...
    __constructor: function(targets, opts) {
        ...
        var configPath = this._configPath = opts.configPath;
    ...
    requirePlugins: function() {
        return _(this._plugins)
            .map(function(config, path) {
                config = utils.getConfig(config, path);

                if(!config) {
                    return false;
                }

                path = resolve.sync(path, {basedir: this._configDir});

                return new Plugin(path, config);
            }, this)
            .compact()
            .value();
    },
    ...
    _resolvePath: function(path) {
        return PATH.resolve(this._configDir, path);
    }
});
