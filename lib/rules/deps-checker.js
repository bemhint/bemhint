...
module.exports = inherit(Checker, {
    ...
    checkEntity: function (entity) {
        var depsJs = entity['deps.js'],
            ...
            expectedDeps = this._getExpectedDeps(entity.js, entity['bemhtml.js'] || entity.bemhtml);

        expectedDeps = _removeSelfDependency(expectedDeps, entityInfo);
    ...
    _getActualDeps: function (depsJs) {
        var deps = depsJs && depsJs.content ? vm.runInThisContext(depsJs.content) : {};
 ...
 * @param {String} entityInfo.path
 * @param {String} entityInfo.basename
 * @returns {Object}
 */
function _removeSelfDependency(deps, entityInfo) {
    var entityBasename = entityInfo.basename,
        parsedEntity = bemNaming.parse(entityBasename),
        typeOfEntity = bemNaming.typeOf(entityBasename);

    if (parsedEntity.block === 'i-bem') {
        if ((typeOfEntity === 'block' || typeOfEntity === 'blockMod') && _.isEmpty(deps.elems)) {
            return {};
        } else if (typeOfEntity === 'elem' || typeOfEntity === 'elemMod') {
            deps.elems = _.without(deps.elems, parsedEntity.elem);
        }
    }
