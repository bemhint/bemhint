...
module.exports = inherit({
    ...
    _checkDeps: function (entity) {
        var entityInfo = utils.getEntityInfo(entity),
            actualDeps = this._getActualDeps(entity['deps.js']),
            expectedDeps = this._getExpectedDeps(entity.js, entity.bemhtml),
            isEqual = this._isEqual(actualDeps, expectedDeps);

        return !isEqual && {
            path: entity['deps.js'] ? entity['deps.js'].path : entityInfo.path,
            actual: entity['deps.js'] ? actualDeps : 'No deps file ' + entityInfo.basename + 'deps.js',
